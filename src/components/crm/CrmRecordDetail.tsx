...
export default function CrmRecordDetail() {
  ...
  const highlights: Array<{ label: string; icon: TodayIconName; value: unknown; muted?: boolean; people?: TeamMember[]; color?: string; currency?: boolean }> = object.slug === 'companies' ? [
    ...
    { label: 'Connection strength', icon: 'heart' as TodayIconName, value: connectionLabel(record.values.connection_strength), color: '#e11d48' },
  ...
  return <div className="rp handoff-record-detail">
    <div className="rp-cols">
      <div className="rp-left" style={{ width: leftWidth }}>
        <div className="rp-left-top">
          <button className="rp-x" onClick={() => navigate(objectListPath(object))}><Icon name="x" size={15} /></button>
          <span className="rp-nav">
            <button disabled={currentIndex <= 0} onClick={() => goToRecord(-1)}><Icon name="caretUp" size={11} /></button>
            <button disabled={currentIndex < 0 || currentIndex >= recordOrder.length - 1} onClick={() => goToRecord(1)}><Icon name="caretDown" size={11} /></button>
          </span>
        </div>
        ...
        <div className="rp-left-scroll">
          {leftMode === 'search' ? <>
            <div className="rp-panel-hd"><button className="rp-collapse" onClick={() => { setLeftMode('details'); setQuery('') }}><Icon name="caretLeft" size={13} /></button><div className="pop-search" style={{ flex: 1, border: '1px solid var(--border-1)', borderRadius: 'var(--rc)' }}><span className="ico"><Icon name="search" size={13} /></span><input autoFocus value={query} onChange={event => setQuery(event.target.value)} placeholder="Search attributes..." /></div></div>
    ...
    {modal === 'compose' && <div className="scrim" onClick={() => setModal(null)}>
      <div className="rm rm-email" onClick={event => event.stopPropagation()}>
        <div className="rm-hd"><button className="rm-ico" onClick={() => setModal(null)}><Icon name="caretLeft" size={15} /></button><span className="rm-title"><Icon name="mailPlus" size={15} />Compose email</span><span className="rm-hd-r"><button className="rm-ico"><Icon name="minus" size={15} /></button><button className="rm-ico" onClick={() => setModal(null)}><Icon name="x" size={15} /></button></span></div>
        ...
        <div className="rm-email-toolbar"><span className="rm-tb-icons"><button className="rm-ico"><Icon name="link" size={15} /></button><button className="rm-ico"><Icon name="image" size={15} /></button><button className="rm-ico"><Icon name="brackets" size={15} /></button><button className="rm-ico"><Icon name="pencil" size={15} /></button></span><span className="rm-tb-r"><span className="rm-mass"><span className="tg on"><span className="knob" /></span>Mass sending <Icon name="status" size={12} /></span><button className="rm-ico" onClick={() => setModal(null)}><Icon name="trash" size={15} /></button><button className="btn btn-primary" onClick={() => notify('gmail', 'Send email needs Gmail send scope')}><Icon name="arrowRight" size={13} />Send emails (1)</button></span></div>
