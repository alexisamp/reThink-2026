...
const PALETTE = ['#79D65E', '#2F6DF6', '#22B8CF', '#E8813A', '#C04AD6', '#EC4899', '#E11D48', '#F5A623', '#16A34A', '#8B5CF6', '#64748B', '#8A99A3']
...
export default function CrmViewSurface(props: CrmViewSurfaceProps) {
  ...
  return <div className="lv crm-view-surface">{toolbar}{isKanban ? kanban : table}
    {selected.size > 0 && createPortal(<div className="bulkbar"><span className="bulk-n">{selected.size} Selected</span><button className="bulk-x" onClick={() => setSelected(new Set())}><Icon name="x" size={13} /></button><span className="bulk-sep" /><button className="bulk-act" onClick={() => props.onAddToList?.([...selected])}><Icon name="list" size={13} />Add to list</button><button className="bulk-act" disabled><Icon name="chat" size={13} />Send email</button><button className="bulk-act" disabled><Icon name="sparkle" size={13} />Run workflow</button>{props.onRemoveRecords && <button className="bulk-act danger" onClick={() => void props.onRemoveRecords?.([...selected]).then(() => setSelected(new Set()))}><Icon name="trash" size={13} />Remove from list</button>}</div>, document.body)}
