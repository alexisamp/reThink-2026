...
const GoalWizard: React.FC<{
    ...
    const handleSave = () => {
        ...
        const newHabitsToCreate = habits.map(h => ({
            ...
            goalId: finalGoal.id,
            text: h.text,
            type: h.targetValue ? HabitType.SCALE : HabitType.BINARY,
            frequency: h.frequency,
            defaultTime: h.defaultTime || undefined,
            reward: h.reward || undefined,
            targetValue: h.targetValue ? parseInt(h.targetValue) : undefined,
    ...
    const addHabit = () => {
        if(newHabit.trim()) {
            ...
            setNewHabit(''); setNewHabitTime(''); setNewHabitReward(''); setNewHabitTarget(''); setNewHabitUnit('');
        }
    };
                                     ...
                                     <div className="flex items-start gap-3">
                                         ...
                                         <div>
                                            ...
                                            <div className="text-xs text-notion-dim flex flex-wrap gap-x-3 gap-y-1 mt-1">
                                                ...
                                                {h.targetValue && <span className="flex items-center gap-1 bg-gray-100 px-1 rounded border">Target: {h.targetValue} {h.unit}</span>}
                             ...
                             <div className="bg-notion-sidebar p-4 rounded border border-notion-border space-y-3">
                                 ...
                                 <div>
                                     <input 
                                        ...
                                        placeholder="Habit (e.g. Read, Drink Water)" 
                                        value={newHabit} 
                                 ...
                                 <div className="grid grid-cols-4 gap-2">
                                     <select 
                                         className="bg-white border border-notion-border p-2 rounded text-xs outline-none focus:border-black" 
                                         value={newHabitFreq} 
                                     ...
                                     />
                                     <input 
                                         className="bg-white border border-notion-border p-2 rounded text-xs outline-none focus:border-black placeholder:text-notion-dim" 
                                 ...
                                 <div className="flex gap-2">
                                     <input 
                                         className="flex-1 bg-white border border-notion-border p-2 rounded text-xs outline-none focus:border-black placeholder:text-notion-dim" 
                         ...
                         <div className="space-y-2">
                             {milestones.map((m, i) => (
                                 <div key={i} className="flex items-center gap-3 p-2 border-b border-notion-border last:border-0">
                                     ...
                                     <input 
                                        className="flex-1 bg-transparent text-sm outline-none border-b border-transparent focus:border-black"
                                        value={m.text}
                                        onChange={(e) => {
                                     ...
                                     />
                                     <select 
                                        className="text-xs font-mono bg-notion-sidebar px-2 py-1 rounded text-notion-dim outline-none cursor-pointer"
                                        value={m.targetMonth || 'Jan'}
                                        onChange={(e) => {
                                            const arr = [...milestones];
                                            arr[i].targetMonth = e.target.value;
                                            setMilestones(arr);
                                        }}
                                     >
...
const StrategyTab: React.FC<StrategyTabProps> = ({ 
  ...
  const handleDeleteSubmit = () => {
    if (deleteConfirmation.toLowerCase() === "delete") {
        // NOTE: Strict Schema means we MUST delete everything. 
        // passing true/true to match the old signature, but logic is simplified in storage.
        onDeleteWorkbook(selectedYear, true, true);
      ...
      {showDeleteModal && (
          <div className="fixed inset-0 z-[200] bg-black/20 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md animate-in fade-in zoom-in-95 border border-notion-border">
                  ...
                  <div className="space-y-3 mb-6 bg-red-50 p-4 rounded border border-red-100">
                      <p className="text-xs font-bold uppercase tracking-wider text-red-700 mb-2">WARNING: CASCADING DELETE</p>
                      <p className="text-sm text-red-900">
                          Because the database is strict, deleting this Workbook will also delete 
                          <strong> ALL Goals, Habits, Logs, and Tasks</strong> associated with it.
                      </p>
                              ...
                              <div className="p-6 grid md:grid-cols-2 gap-8">
                                  <div>
                                      <h4 className="text-xs font-bold uppercase tracking-widest text-notion-dim mb-4 flex items-center gap-2">
                                          <Map className="w-3 h-3" /> Roadmap ({goal.milestones.filter(m => m.completed).length}/{goal.milestones.length})
                                      </h4>
