...
export function MeetingModal({ meeting, onClose }: { meeting: MeetingDetail; onClose: () => void }) {
  return createPortal(<div className="scrim" onClick={onClose}><div className="modal sm meeting-modal" onClick={event => event.stopPropagation()}>
    <div className="modal-hd"><Logo id={meeting.logo || meeting.title} size={22} sq /><span>{meeting.title}</span><button className="x" onClick={onClose} aria-label="Close meeting details"><Icon name="x" size={15} /></button></div>
...
export function CloseDayFlow({ stats, unfinished, saving, onClose, onCommit }: { stats: CloseDayStats; unfinished: Todo[]; saving: boolean; onClose: () => void; onCommit: (payload: { carryIds: string[]; objective: string }) => void }) {
  ...
  return createPortal(<div className="closeday-screen"><div className="cd-top"><span className="cd-steps"><span className={`cd-dot${step === 'recap' ? ' on' : ''}`} /><span className={`cd-dot${step === 'plan' ? ' on' : ''}`} /></span><button className="x" onClick={onClose} aria-label="Close day review"><Icon name="x" size={16} /></button></div>
