...
export default function ObjectRecords() {
  ...
  const stages = useMemo<CrmViewStageSetting[]>(() => {
    ...
    return (attribute?.options ?? []).map((option, index) => ({ id: option.id, label: option.label, color: option.color || ['#79D65E', '#2F6DF6', '#22B8CF', '#E8813A'][index % 4] }))
    ...
    <div className="rp handoff-record-detail">
      <div className="rp-cols">
        <div className="rp-left" style={{ width: leftWidth }}>
          <div className="rp-left-top"><button className="rp-x" onClick={() => navigate(objectListPath(object))}><Icon name="x" size={15} /></button><span className="rp-nav"><button disabled><Icon name="caretUp" size={11} /></button><button disabled><Icon name="caretDown" size={11} /></button></span></div>
