create or replace function public.review_lookup_text(value text)
returns text
language sql
immutable
as $$
  select trim(regexp_replace(
    lower(translate(coalesce(value, ''), 'áéíóúàèìòùäëïöüñÁÉÍÓÚÀÈÌÒÙÄËÏÖÜÑ', 'aeiouaeiouaeiounAEIOUAEIOUAEIOUN')),
    '[^a-z0-9]+',
    ' ',
    'g'
  ));
...
create or replace function public.review_digits(value text)
returns text
language sql
immutable
as $$
...
declare
  ...
  wa_name text := nullif(payload ->> 'wa_name', '');
  ...
  interaction_type text := 'whatsapp';
  interaction_channel text := 'whatsapp';
...
drop trigger if exists review_items_auto_accept_conversations on public.review_items;
create trigger review_items_auto_accept_conversations
  after insert or update of proposed_payload, contact_id, status
  on public.review_items
  for each row
  execute function public.auto_accept_conversation_interaction_review_item();

-- Repair Monica's known phone from the LinkedIn handoff message so future
-- WhatsApp captures resolve by phone even when WhatsApp only sends the chat id.
with monica as (
  select id, user_id
  from public.outreach_logs
  where public.review_lookup_text(name) like 'monica castillo%'
)
update public.outreach_logs contact
set phone = coalesce(contact.phone, '+16179496082'),
    email = coalesce(contact.email, 'mkco11@gmail.com'),
    updated_at = now()
from monica
where contact.id = monica.id
  and contact.user_id = monica.user_id;
...
select user_id, id, '+16179496082', 'WhatsApp', 'manual-backfill:monica-castillo-whatsapp', now()
from public.outreach_logs
where public.review_lookup_text(name) like 'monica castillo%'
on conflict (user_id, phone_number) do update
set contact_id = excluded.contact_id,
...
-- Re-run the trigger for pending Conversations WhatsApp items now that phone
-- and name resolution are available.
update public.review_items
set status = status,
    updated_at = now()
